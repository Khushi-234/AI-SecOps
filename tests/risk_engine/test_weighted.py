...
Tests public behavior of WeightedScorer:
    ...
    - Empty evidence and null payload fail-secure validation (InvalidRiskInputError)
...
def test_weighted_scorer_single_finding(scorer: WeightedScorer, sample_evidence: RiskEvidence):
    """Verify scoring a single finding returns a complete, valid RiskScore object."""
    # Act
    ...
    assert result.metadata.get("scorer_class") == "WeightedScorer"


def test_weighted_scorer_multiple_findings(scorer: WeightedScorer):
    """Verify scoring multiple findings computes the expected weighted sum and valid RiskScore model."""
    # Arrange
    ev1 = RiskEvidence(
        evidence_id="ev-1",
        source_module="PROMPT_FIREWALL",
...
def test_weighted_scorer_empty_evidence_raises_invalid_input(scorer: WeightedScorer):
...
def test_weighted_scorer_none_evidence_raises_invalid_input(scorer: WeightedScorer):
...
def test_weighted_scorer_invalid_evidence_item_raises_invalid_input(scorer: WeightedScorer):
