...
def test_detector_failure_fail_secure_isolation(
    audit_logger: FakeAuditLogger, normalizer: TextNormalizer, context: dict[str, Any]
) -> None:
    """Verifies pipeline continues under fail_secure=True and halts under fail_secure=False."""
    # Arrange
    failing_detector = FakeDetector(
        "FaultyDetector", should_raise=ValueError("Mocked crash")
    )
...
def test_latency_tracking_non_negative(
    ...
    # Act
    response = firewall.inspect_prompt(
        "Simple timing verification prompt", context=context
    )
