...
def test_sprint6_framework() -> None:
    ...
    config = InputValidatorConfig(
        ...
        language=LanguageConfig(
            supported_languages=["en", "hi"], allow_unknown_languages=False
        ),
        schema=SchemaConfig(
    ...
    validator = InputValidator(
        config=config, validators=[empty_val, length_val, schema_val, lang_val]
    )
    ...
    for r in res_1.results:
        print(
            f"  - {r.validator_name}: is_valid={r.is_valid}, error_message={r.error_message}, metadata={r.metadata}"
        )
    ...
    for r in res_2.results:
        print(
            f"  - {r.validator_name}: is_valid={r.is_valid}, error_message={r.error_message}"
        )
    ...
    prompt_8 = (
        '{"user_id": "usr_1", "prompt": "The quick brown fox jumps over the lazy dog"}'
    )
