"""
Purely reactive security rules for Secret and Credential protection.
"""
...
from prompt_hardener.constraints import SECRET_PROTECTION_CONSTRAINT
from prompt_hardener.rules.base import BaseSecurityRule


class SecretSecurityRule(BaseSecurityRule):
    """
    Applies secret protection constraint strictly based on RiskContext metadata.
    ...
    def evaluate(self, risk_context: Any, policy_decision: Any) -> list[str]:
        ...
        has_secret_threat = (
            any(kw in attack_type for kw in secret_keywords)
            or any(any(kw in t for kw in secret_keywords) for t in threats)
        )

        if has_secret_threat:
            constraints.append(SECRET_PROTECTION_CONSTRAINT)
